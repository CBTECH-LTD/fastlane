<template>
    <inertia-link
        ...
        :class="classes">
        <span v-if="item.icon" class="flex text-2xl font-semibold justify-center w-16">
            <f-icon :name="item.icon"/>
        </span>
        <span class="text-sm font-medium opacity-100" :class="item.icon ? '' : 'pl-8'">{{ item.label }}</span>
    </inertia-link>
</template>
